...
| 3 | **Explicit approval for every edit and command** | **REJECT as default** | It is Cline's stated core trust proposition and it is genuinely safe — but the documented cost is speed, and we already buy that safety from the OS sandbox. Available as a permission mode; never the default. |
